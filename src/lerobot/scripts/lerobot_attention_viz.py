...
def _extract_view_images(batch: dict[str, Any]) -> list[torch.Tensor]:
    """Return all OBS_IMAGES-prefixed tensors in deterministic key order."""
    keys = sorted(k for k in batch if k.startswith(OBS_IMAGES + "."))
...
def _capture_attention_for_snapshot(
    ...
    with AttentionRecorder(transformer, layer_indices) as recorder, torch.no_grad():
...
def attention_viz_main(cfg: AttentionVizPipelineConfig):
    ...
    # ── Auto-detect view count if not specified ───────────────────────
    sample_views = _extract_view_images(snapshots[0][0])
    n_views = cfg.analysis.n_views or len(sample_views)
    if n_views == 0:
        raise RuntimeError("Could not auto-detect any OBS_IMAGES tensors in the batch.")
    view_token_ranges = _compute_view_token_ranges(n_views, cfg.analysis.patches_per_view)
    patches_per_view = [cfg.analysis.patches_per_view] * n_views
    ...
    for frame_idx in frame_iter:
        ...
        images = _extract_view_images(batch_cpu)[:n_views]
        ...
        save_path = output_dir / f"attention_frame{frame_idx:04d}.png"
        plot_overlay_panel(
            images_per_view=images,
            attentions_per_condition=attentions,
            ...
            suptitle=(
                ...
                f"frame {frame_idx}, query pos {cfg.analysis.query_position}, head={cfg.analysis.head_reduction}"
            ),
        )
        ...
        if cfg.analysis.plot_text_bar and tokenizer is not None:
            ...
            if lang_ids is None:
                ...
                if positions:
                    # Language tokens follow all vision tokens in the prefix.
                    language_token_start = sum(p_h * p_w for (p_h, p_w) in patches_per_view)
                    bar_save_path = output_dir / f"text_attention_frame{frame_idx:04d}.png"
                    plot_text_attention_bar(
                        attentions_per_condition=attentions,
                        query_index=cfg.analysis.query_position,
